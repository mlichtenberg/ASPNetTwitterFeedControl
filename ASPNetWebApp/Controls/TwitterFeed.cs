...
namespace ASPNetWebApp
{
    public class TwitterFeed
    {
        ...
        {
            string cacheKey = "TwitterFeedClassCacheKey";
            StringBuilder twitterFeedContents = new StringBuilder();

            if (HttpRuntime.Cache[cacheKey] != null)
            {
                // Used the cached version of the feed contents
                twitterFeedContents.Append(HttpRuntime.Cache[cacheKey].ToString());
            }
            else
            {
                twitterFeedContents.Append("<div class=\"BlackHeading\" style=\"text-align:left\">" + this.twitterUserID + " on <a style=\"text-decoration:none\" href=\"http://twitter.com/" + this.TwitterUserID + "\" target=\"_blank\">Twitter</a></div>");
                twitterFeedContents.Append("<div class=\"twtr-widget twtr-widget-profile\">");
                twitterFeedContents.Append("<div class=\"twtr-doc\">");
                twitterFeedContents.Append("<div class=\"twtr-timeline\" style=\"height:120px; overflow: auto; overflow-y:scroll; overflow-x:hidden;\">");
                twitterFeedContents.Append("<div class=\"twtr-tweets\">");
                    ...
                    foreach (object result in results)
                    {
                        ...
                        foreach (var h in hashTags)
                        {
                            ...
                            string replaceHashText = string.Format(
                                ...
                                HttpUtility.UrlEncode(hashText), hashText);
                ...
                HttpRuntime.Cache.Add(cacheKey, twitterFeedContents, null, DateTime.Now.AddMinutes(twitterFeedCacheTime),
                    System.Web.Caching.Cache.NoSlidingExpiration, System.Web.Caching.CacheItemPriority.Normal, null);
            }

            twitterFeedContents.Append("</div></div></div></div>");

            // Output the twitter feed contents
            return twitterFeedContents.ToString();
        }
    }
}